#include <iostream>
#include "../underlying code/stack.cpp"
#include <string>
#include <cmath>
#include <cstdlib>
#include <ctime>
...
// 扩展: 支持三角函数和对数
double evaluateTrigFunction(const string &expression) {
...
// 4. 随机生成测试数据
void generateRandomData(int numTests) {
    srand(time(0)); 
    for (int i = 0; i < numTests; ++i) {
        ...
        cout << "]\nMax Area: " << largestRectangleArea(heights) << endl << endl;
    }
}
int main()
{
    ...
    string A3 = {"sin(30)"};
    cout << "数字运算: " << A3 << " = " << evaluateTrigFunction(A3) << endl<< endl;
    // 测试柱状图最大矩形面积
    vector<int> B1 = {2, 1, 5, 6, 2, 3};
    vector<int> B2 = {2, 4};
    cout << "输入: " << "[2, 1, 5, 6, 2, 3]" << endl;
    cout << "输出: " << largestRectangleArea(B1) << endl <<endl;
    cout << "输入: " << "[2, 4]" << endl;
    cout << "输出: " << largestRectangleArea(B2) << endl << endl;
    // 随机生成 10 组测试数据
